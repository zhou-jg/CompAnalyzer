...
public class ClassFileParser extends AbstractParser {
    ...
    private String parseSuperClassName() throws IOException {
        ...
        if (addImport(getPackageName(superClassName))){
        	jClass.setSuperClazz(addReference(getClassName(superClassName)));
        };
    ...
    private String[] parseInterfaces() throws IOException {
        ...
        for (int i = 0; i < interfacesCount; i++) {
            ...
            if (addImport(getPackageName(interfaceNames[i]))){
            	jClass.addInterface(addReference(getClassName(interfaceNames[i])));
            }
    ...
    private FieldOrMethodInfo[] parseFields() throws IOException {
        ...
        for (int i = 0; i < fieldsCount; i++) {
            ...
            for (int t = 0; t < types.length; t++) {
                if (addImport(getPackageName(types[t]))){
                	addReference(getClassName(types[t]));
                }
    ...
    private void addClassConstantReferences() throws IOException {
        for (int j = 1; j < constantPool.length; j++) {
            if (constantPool[j].getTag() == CONSTANT_CLASS) {
                ...
                if (addImport(getPackageName(name))){
                	addReference(getClassName(name));
                }
    ...
    //true indicates the importedPackage in the scope; false otherwise. 
    private boolean addImport(String importPackage) {
    ...
    private JavaClass addReference(String refClass){
    	JavaClass ret = new JavaClass(refClass);
    	jClass.addReferencedClass(ret);
    	return ret;
    }
